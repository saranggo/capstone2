function model = train_c(conf, model, pos, neg, warp, randneg, iter, ...
  ...
  if ~cont || t > 1
    ...
    if warp
      ...
      [num_entries_added, num_examples_added, fusage, component_usage, scores] ...
          = poslatent(conf, t, iter, model, pos, fg_overlap, num_fp);
      th = toc(th);
      ...
      if randneg
        ...
        [num_entries_added, num_examples_added, ...
         negpos, fusage, scores, complete] ...
            = neghard(conf, tneg, negiter, model, neg, bytelimit, ...
...
function [num_entries, num_examples, j, fusage, scores, complete] ...
  = neghard(conf, t, negiter, model, neg, maxsize, negpos, max_num_examples)
